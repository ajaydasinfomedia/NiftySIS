			...
			<table id="exportlist" class="table table-striped" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<tr>
						...
						."&nbsp;".
						$this->Form->button( __('Delete'),['action'=>'#','url'=>$this->request->base.'/'.$this->name.'/import_delete/'.$get_data['export_id'],'class'=>'btn btn-danger sa-warning']);
...
jQuery("body").on("click", ".edit-term", function(){
	...
	var curr_data = {					
			...
			jQuery.ajax({
				type:"POST",
				url:"<?php echo Router::url(array('controller'=>'Export','action'=>'importedit'));?>",
				data:curr_data,
				async:false,
				success: function(response){ 								   
