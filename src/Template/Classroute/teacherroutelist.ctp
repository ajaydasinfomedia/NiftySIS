			...
			<?php 
				...
				if(isset($teacher_name))
				{
					...
					<div class="panel panel-default">
						...
						<?php echo __('Teacher : '); ?> <?php echo __($teacher['teacher_name']);?> </a>
												...
												{ 	
													...
													{																
														foreach($period_data[$key2] as $p_data)
															{
																if($p_data['teacher'] ==  $teacher['user_id'] && $p_data['day'] == $period )
																{
																	...
																	echo "<li>".$this->Html->link(__('Edit'),array('controller'=>'Classroute','action' => 'updateteacherroute', $this->Setting->my_simple_crypt($p_data['route_id'],'e')),array('class'=>''))."</li>";
																	echo "<li>".$this->Form->button( __('Delete'),['action'=>'#','url'=>$this->request->base.'/'.$this->name.'/delete/'.$p_data['route_id'],'class'=>'sa-warning'])."</li>";
																	
																	 /*  
																	<li><a href="updateteacherroute/'.$p_data['route_id'].'">'.__('Edit').'</a></li>
																	<li><a href="delete/'.$p_data['route_id'].'">'.__('Delete').'</a></li>
																	 */ 	
