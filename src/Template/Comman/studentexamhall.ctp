...
$(document).ready(function(){
	...
	$('#abc').on('click',function(){
		...
		{			
			...
			data={h_id:get_id,exam_id:exam_id,hall_id:hall_id};	
			
			jQuery.ajax({
				type:"POST",
				url:"<?php echo Router::url(array('controller'=>'Comman','action'=>'assgnexamhall'));?>",
				data:data,
				async:false,
	...
	$("body").on('click','.btn_del',function(){
		...
		$.ajax({
			type:'POST',
			url:"<?php echo Router::url(array('controller'=>'Comman','action'=>'removeexamhall'));?>",
...
<div class="form-group">
	<div class="table-responsive">
		...
		if(isset($student_data) || isset($student_data1))
		{
			...
			<tr>
				<td><button type="button" class="btn btn-success" id="abc"><?php echo __('Assign Exam Hall');?> </button></td>
			</tr>
			<?php
			}
			else{
