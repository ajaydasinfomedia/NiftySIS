...
<div class="default_main">
	...
	{
		if($student_menu['teacher_approve'] == '1')
		{
		?>
	...
	<div class="row">
		<div class="responsivesortdata col-md-7 col-sm-7 col-xs-12">				
			<?php
			$access = $this->Setting->check_user_access_module('Class Routine','teacher');
			if($access):
				...
				<div class="row">
					<div class="panel panel-white classrouting">
						<div class="panel-heading" style="margin-bottom: 15px;">
							...
							<?php echo __('Class : '); ?> 
								<?php echo $this->Setting->get_class_id($classes);?>
													...
													<?php
														foreach($class_route as $period_data)
														{
															if(array_key_exists($key2,$period_data))
															{
																...
																{
																	if($p_data['class'] ==  $classes && $p_data['teacher'] ==  $teacher_array['user_id'] && $p_data['day'] == $period )
																	{
			...
			<?php
			$access = $this->Setting->check_user_access_module('Report','teacher');
			if($access):
			...
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="row">
					<div class="panel panel-white report">	
						...
						?>
						<div id="chart_div" style="width: 100%; height: 300px;margin-top:0px;"></div>
						</div>						
					</div>
				</div>
			</div>
			...
			<?php endif; endif; ?>
			<?php
			$access = $this->Setting->check_user_access_module('Message','teacher');
			if($access):
			if(isset($inboxdata)):
			?>
			<div class="col-md-12 col-sm-12 col-xs-12">
		...
		<div class="responsivesortdata col-md-5 col-sm-5 col-xs-12">
			<?php
			$access = $this->Setting->check_user_access_module('Homework','teacher');
			if($access){
			if(isset($it)){
			?>
			...
			$access = $this->Setting->check_user_access_module('Exam','teacher');
			if($access){
			if(isset($exam_list)){
			?>
			...
			$access = $this->Setting->check_user_access_module('Notice Board','teacher');
			if($access){					
			...
			$access = $this->Setting->check_user_access_module('News','teacher');
			if($access){
			if(isset($news_list)){
			?>
			...
			$access = $this->Setting->check_user_access_module('Holiday','teacher');
			if($access){
			...
			$access = $this->Setting->check_user_access_module('Event','teacher');
			if($access){
			if(!empty($event_list)){
			?>
			...
			<?php
					...
					echo "<div class='panel panel-white event'><div class='panel-heading'><h4 class='text-danger'>Not Available Any Event</h4></div></div>";
			}
			?>
		</div>
		<?php $this->Form->end(); ?>
