...
$(document).ready(function(){
	$('body').on('change', '#exam_id', function() {			
		...
		else
		{
			...
			jQuery.ajax({
				type:"POST",
				url:"<?php echo Router::url(array('controller'=>'Comman','action'=>'studentexamhall'));?>",
				data:curr_data,
				async:false,
...
<div class="row">			
	<ul role="tablist" class="nav nav-tabs panel_tabs">
		<li class="active">
			<?php  echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-list fa-lg')) . __('Exam Hall Receipt'),array('controller'=>'Comman','action' => 'examhallreceipt'),array('escape' => false));?>
		</li>
	</ul>
</div>
