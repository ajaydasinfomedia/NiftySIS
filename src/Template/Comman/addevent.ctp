...
<div class="row">			
	<ul role="tablist" class="nav nav-tabs panel_tabs">
		...
		<?php  
		if(isset($edit))
			echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-plus-circle fa-lg')) . __('Edit Event'),array('controller'=>'Comman','action' => 'addevent',$row['event_id']),array('escape' => false));
		else
			echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-plus-circle fa-lg')) . __('Add Event'),array('controller'=>'Comman','action' => 'addevent'),array('escape' => false));					
		?>
		</li>
	</ul>
</div>
			...
			<?php echo $this->Form->Create('form1',['id'=>'formID','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'addnews']]);?>
				...
				<div class="form-group">
					...
					<div class="col-md-10 col-sm-10 col-xs-12 datepickericon">
						...
						echo $this->Form->input('',array('name'=>'start_date','value'=>$start_date,'id'=>'date_of_birth','class'=>'form-control validate[required]','PlaceHolder'=> __('Enter News date ')));?>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-md-10 col-sm-10 col-xs-12 datepickericon">
						...
						if(isset($edit))
							...
							$end_date = date('Y-m-d');
						echo $this->Form->input('',array('name'=>'end_date','value'=>$end_date,'id'=>'date_of_birth1','class'=>'form-control validate[required]','PlaceHolder'=> __('Enter News date ')));?>
					</div>
				</div>
