<script>
		...
		jQuery("#dep").change(function(){
			...
			$.ajax({
				...
				type:'POST',
				url:'<?php echo $this->Url->build(["controller"=>"Marks","action"=>"showdata"])?>',
...
<div class="row">			
		...
		<div class="mark_tbl"> 
			...
			<div class="col-md-12 col-sm-12 col-xs-12">		
				<?php echo $this->Form->label(__('Select CSV File'));?>		
				<?php echo $this->Form->input('',array('type'=>'file','name'=>'csv_file'));?>		
				<?php echo $this->Form->input(__('Fill data from CSV File'),array('type'=>'submit','name'=>'upload_csv_file','class'=>'btn btn-info'));?>
				<a class="btn btn-primary submit" href="<?php echo $this->request->webroot; ?>Reports/import_marks.csv" style="margin-left:5px;"><?php echo __('Sample Download');?></a>			
			</div>
			<div class="col-md-12 col-sm-12 col-xs-12">
				...
				<div id="example_wrapper" class="dataTables_wrapper">
					...
					<tbody>						
						...
						foreach($user as $user_data)
						{
							...
							echo "<td>" ."<button type='submit' name='add_mark' value='".$user_data['user_id']."' class='btn btn-success'>"?> <?php echo __('Update'); ?> <?php "</button>"."</td>";							
					...
					<div class="col-md-12 col-sm-12 col-xs-12">
						<div class="pull-right">
							<?php echo $this->Form->input(__('Update All Marks'),array('type'=>'submit','name'=>'save_all_marks','class'=>'btn btn-success'));?>
						</div>
					</div>
					</div>
					</div>
							...
							{ 
								...
								{								
									...
									{								
										...
										echo "<td>" . $name . "</td>";
										?>	
										<td><input type="text" name="marks_<?php echo $user_data['user_id'];?>" value="<?php echo $csv['marks'];?>" class="form-control validate[required,custom[onlyNumberSp]] text-input"></td>
										<td><input type="text" name="marks_comment_<?php echo $user_data['user_id'];?>" value="<?php echo $csv['comment'];?>" class="form-control validate[required,custom[onlyNumberSp]] text-input"></td>							
								...
								if($y == 0 	&& ($csv['roll'] != $user_data['roll_no']))
								{
									$x=0;						
									$name=$user_data['first_name']." ".$user_data['last_name'];
									echo "<tr>";
									...
									foreach($marktabel as $marktabeldata)	
									{
										...
										{								
											?>	
											<td><input type="text" name="marks_<?php echo $user_data['user_id'];?>" value="<?php echo $marktabeldata['marks'];?>" class="form-control validate[required,custom[onlyNumberSp]] text-input"></td>
											<td><input type="text" name="marks_comment_<?php echo $user_data['user_id'];?>" value="<?php echo $marktabeldata['marks_comment'];?>" class="form-control validate[required,custom[onlyNumberSp]] text-input"></td>							
											<?php
											$x=1;	
											break;						
									...
									if($x==0){
										...
										echo "<td>" ."<input type='text' name='marks_".$user_data['user_id']."' class='form-control validate[required,custom[onlyNumberSp]] text-input'>" . "</td>";
										echo "<td>" ."<input type='text' name='marks_comment_".$user_data['user_id']."' class='form-control validate[required,custom[onlyNumberSp]] text-input'>" . "</td>";
									}						
									echo "<td>" ."<button type='submit' name='add_mark' value='".$user_data['user_id']."' class='btn btn-success'>"?> <?php echo __('Update'); ?> <?php "</button>"."</td>";
									?>
