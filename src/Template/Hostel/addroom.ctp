		...
		closeOnCancel: false
			...
			if (isConfirm)
			{
				...
				$.ajax({
				   ...
				   data:{category_id:id},
				   success:function(data){
						$("#room_category option[value="+id+"]").remove();
		...
		<div class="panel-body">
			<?php echo $this->Form->Create('form1',['id'=>'formID','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'addhostel']]);?>
				...
				<div class="form-group">
					...
					<div class="col-md-8 col-sm-8 col-xs-12 export_model">
						<select class="form-control validate[required,maxSize[50]]" id="room_category" name="room_category">
							...
							?>
							<option value="<?php echo $fetch_data['hostel_room_category_id'];?>" class="room_cat_<?php echo $fetch_data['hostel_room_category_id'];?>" <?php echo $selected;?>>
				...
				<div class="form-group">
					...
					<div class="col-md-8 col-sm-8 col-xs-12">
						<?php echo $this->Form->input('',array('name'=>'beds_capacity','value'=>$beds_capacity,'class'=>'form-control validate[required,custom[onlyNumberSp],maxSize[3]]','PlaceHolder'=> __('Enter Bed capacity')));?>
					</div>
				</div>
