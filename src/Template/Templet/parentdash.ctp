...
<div class="default_main">
	...
	{
		if($student_menu['parent_approve'] == '1')
		{
		?>
		...
		<div class="responsivesort col-lg-3 col-md-3 col-sm-6 col-xs-12">
			<div class="panel info-box panel-white">
				<div class="panel-body attendence">
					...
					<div class="info-box-stats">				
						<?php  echo $this->Html->link(__('Today Attendance'),['controller' => 'Comman', 'action' => 'childlist'],['class' =>'attenddash']);?>
	...
	<div class="row">
		<div class="responsivesortdata col-md-7 col-sm-7 col-xs-12">
			<?php
			$access = $this->Setting->check_user_access_module('Message','parent');
			if($access):
			if(isset($inboxdata)):
			?>
			...
			<?php endif; endif; ?>
			<?php
			$access = $this->Setting->check_user_access_module('Class Routine','parent');
			if($access):
			if(isset($class_route) && isset($class_list_parent_child)):		
			?>
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="row">
					<div class="panel panel-white classrouting">
						...
						foreach($class_list_parent_child as $class_id)
						{ 	
							// $class_id = $this->Setting->get_user_class($user);
						?>
						<div class="panel-heading" style="margin-bottom: 15px;">
							<?php echo $this->Html->image('icons/new_class-route.png');?>  
							<h3 class="panel-title">
								<?php echo __('Class : '); ?> 
									<?php echo $this->Setting->get_class_id($class_id);?>
													...
													{ 	
														...
														{																
															foreach($period_data[$key2] as $p_data)
																{
																	if($p_data['class'] ==  $class_id && $p_data['day'] == $period )
																	{
		...
		<div class="responsivesortdata col-md-5 col-sm-5 col-xs-12">
			<?php
			$access = $this->Setting->check_user_access_module('Homework','parent');
			if($access){
			...
			<div class="panel panel-white homework">
				...
				echo __('Child : ');
				echo $this->Setting->get_user_id($user);
			...
			$access = $this->Setting->check_user_access_module('Exam','parent');
			if($access){
			if(isset($exam_list)){
			?>
			...
			$access = $this->Setting->check_user_access_module('Notice Board','parent');
			if($access){					
			...
			$access = $this->Setting->check_user_access_module('News','parent');
			if($access){
			if(isset($news_list)){
			?>
			...
			$access = $this->Setting->check_user_access_module('Holiday','parent');
			if($access){
			...
			$access = $this->Setting->check_user_access_module('Event','parent');
			if($access){
			if(!empty($event_list)){
			?>
