...
<div class="row">			
	<ul role="tablist" class="nav nav-tabs panel_tabs">
		...
		<li class="active">
			<?php  
			if(isset($edit))
				echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-plus-circle fa-lg')) . __('Edit Export'),array('controller'=>'Comman','action' => 'addexport',$this->Setting->my_simple_crypt($row['export_id'],'e')),array('escape' => false));
			else
		...
		<div class="panel-body">
			<?php echo $this->Form->Create('form1',['id'=>'formID','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'addexport']]);?>
				...
				<div class="form-group">
					...
					<?php 
					if(isset($edit))
						echo $this->Form->input(__('Update Export'),array('type'=>'submit','name'=>'add','class'=>'btn btn-success'));
					else
