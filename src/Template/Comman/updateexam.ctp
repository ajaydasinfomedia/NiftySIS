...
$(document).ready(function(){
   ...
   $("#exam_end_date").datepicker({     
	...
	$('body').on('click', '#save', function() {
		 ...
		 $.ajax({
			type: 'POST',
			url: '<?php echo $this->Url->build(["controller" => "Comman","action" => "examadddata"]);?>',
			data : {term_name:str},
			success: function (data)
			{					
				if(data != 'false')
	   ...
	   swal({   
			...
			closeOnCancel: false
				...
				if (isConfirm)
				{
					...
					$.ajax({
					   type:'POST',
					   url:'<?php echo $this->Url->build(['controller'=>'Comman','action'=>'term_delete']);?>',
					   data:{term_id:id},
					   success:function(data){
...
<div class="row">			
	<ul role="tablist" class="nav nav-tabs panel_tabs">
		<li class="active">
			<?php  echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-pencil-square-o fa-lg')) . __('Edit List'),['controller'=>'Comman','action' => 'updateexam',$this->Setting->my_simple_crypt($row['exam_id'],'e')],['escape' => false]);?>
		</li>
		<li>
